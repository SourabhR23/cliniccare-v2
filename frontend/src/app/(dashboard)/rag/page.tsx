...
function TypingIndicator() {
  return (
    <div className="flex items-start gap-3">
      ...
      <div className="bg-[#e8f2f6] border border-[#c8dde6] rounded-[14px] rounded-tl-none px-4 py-3">
...
function SourceItem({ source, allSources }: { source: RAGSource; allSources: RAGSource[] }) {
  ...
  return (
    <div className="bg-white border border-[#c8dde6] rounded-[8px] p-2.5">
      <div className="flex items-center justify-between gap-2 mb-1">
        <span className="font-sans text-[10px] text-sky">{formatDate(source.visit_date)}</span>
        <div className="flex items-center gap-1">
          <div className="w-12 h-1 rounded-full bg-[#e8f2f6] overflow-hidden">
            <div className="h-full rounded-full bg-sky/50" style={{ width: `${pct}%` }} />
          </div>
          <span className="font-sans text-[9px] text-[#8aaab8]">{pct}%</span>
        </div>
      </div>
      <p className="text-[11px] font-medium text-[#052838] leading-snug mb-1">{source.diagnosis}</p>
      <div className="flex items-center gap-1.5">
        <Badge variant="muted" className="text-[8px]">{source.visit_type}</Badge>
        <span className="text-[9px] text-[#8aaab8]">{source.doctor_name}</span>
      </div>
    </div>
  )
}
...
function AgentLabel({ agent, cached }: { agent?: string; cached?: boolean }) {
  ...
  if (isCalendar) {
    return (
      <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-[4px] bg-purple-500/10 border border-purple-500/20 text-purple-400 text-[8px] font-sans uppercase tracking-wider">
        CALENDAR
      </span>
    )
  }
  ...
  return (
    <div className="flex items-center gap-1.5">
      <span className="inline-flex items-center px-1.5 py-0.5 rounded-[4px] bg-sky/10 border border-sky/20 text-sky text-[8px] font-sans uppercase tracking-wider">
        RAG
      </span>
...
function MarkdownContent({ content }: { content: string }) {
  return (
    <ReactMarkdown
      components={{
        h1: ({ children }) => <h1 className="text-base font-semibold text-[#052838] mb-2 mt-3 first:mt-0">{children}</h1>,
        h2: ({ children }) => <h2 className="text-sm font-semibold text-[#052838] mb-1.5 mt-3 first:mt-0">{children}</h2>,
        h3: ({ children }) => <h3 className="text-xs font-semibold text-[#5a8898] uppercase tracking-wider mb-1.5 mt-3 first:mt-0">{children}</h3>,
        p:  ({ children }) => <p className="mb-2 last:mb-0 leading-relaxed">{children}</p>,
        strong: ({ children }) => <strong className="font-semibold text-[#052838]">{children}</strong>,
        em:     ({ children }) => <em className="italic text-[#5a8898]">{children}</em>,
        ...
        code: ({ children }) => <code className="font-sans text-xs bg-[#e8f2f6] rounded px-1 py-0.5 text-sky/80">{children}</code>,
        hr:   () => <hr className="border-[#c8dde6] my-3" />,
      }}
    >
      {content}
    </ReactMarkdown>
    ...
    <div className="flex items-start gap-3">
      ...
      <div className="flex-1 min-w-0 max-w-[85%]">
        <div className="flex items-center gap-2 mb-1.5">
          <span className="text-[10px] font-medium text-[#5a8898] uppercase tracking-wider">
            ClinicCare AI
          </span>
          <AgentLabel agent={message.agent} cached={message.cached} />
          {!isCalendar && message.retrieval_count != null && message.retrieval_count > 0 && (
            <span className="text-[9px] font-sans text-[#8aaab8]">
        ...
        <div className={cn(
          ...
          isCalendar
            ...
            : 'bg-[#e8f2f6] border border-[#c8dde6]'
        )}>
          <div className="px-4 py-3 text-sm text-[#052838] leading-relaxed">
            <MarkdownContent content={message.content} />
          </div>

          {hasSources && (
            <div className="border-t border-[#c8dde6]">
              <button
                onClick={() => setSourcesOpen(v => !v)}
                className="w-full flex items-center gap-2 px-4 py-2 text-[10px] font-sans text-[#8aaab8] hover:text-sky/70 transition-colors"
              >
                <svg
  ...
  return (
    <div className="flex justify-end">
      <div className="max-w-[75%] bg-sky/10 border border-sky/20 rounded-[14px] rounded-tr-none px-4 py-3">
        <p className="text-sm text-[#052838] leading-relaxed">{content}</p>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-7rem)]">
      ...
      <div className="flex items-center justify-between gap-4 mb-4 flex-shrink-0">
        <div>
          <h2 className="text-lg font-semibold text-[#052838]">AI Assistant</h2>
          <div className="flex items-center gap-2 mt-0.5">
            <p className="text-xs text-[#5a8898] font-sans">Multi-turn</p>
            <span className="text-[#8aaab8]">·</span>
            <span className="inline-flex items-center px-1.5 py-0.5 rounded-[4px] bg-sky/10 border border-sky/20 text-sky text-[8px] font-sans uppercase tracking-wider">RAG</span>
            <span className="text-[#8aaab8]">·</span>
            <span className="inline-flex items-center px-1.5 py-0.5 rounded-[4px] bg-purple-500/10 border border-purple-500/20 text-purple-400 text-[8px] font-sans uppercase tracking-wider">CALENDAR</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          ...
          <div className="relative">
            {selectedPatient ? (
              <div className="flex items-center gap-2 bg-[#e8f2f6] border border-sky/25 rounded-[10px] px-3 py-1.5">
                <div className="w-1.5 h-1.5 rounded-full bg-sky animate-pulse" />
                <span className="text-xs font-medium text-[#052838] max-w-[140px] truncate">{selectedPatient.name}</span>
                <button
                  onClick={() => { setSelectedPatient(null); setPatientSearch('') }}
                  className="text-[#5a8898] hover:text-[#052838] transition-colors ml-1"
                >
              ...
              <div className="relative">
                <input
                  ...
                  className="w-48 bg-white text-[#052838] placeholder-[#8aaab8] border border-[#c8dde6] rounded-[10px] px-3 py-1.5 text-xs focus:outline-none focus:border-sky/50 transition-all"
                />
                {searchLoading && (
                ...
                {showPatientDropdown && patientResults && patientResults.length > 0 && (
                  <div className="absolute top-full right-0 mt-1.5 z-30 w-64 bg-white border border-[#c8dde6] rounded-[12px] shadow-2xl overflow-hidden max-h-48 overflow-y-auto">
                    {patientResults.map(p => (
                      <button
                        key={p.id}
                        onMouseDown={() => { setSelectedPatient(p); setPatientSearch(''); setShowPatientDropdown(false) }}
                        className="w-full text-left px-4 py-2.5 hover:bg-[#e8f2f6] transition-colors border-b border-[#c8dde6] last:border-0"
                      >
                        <p className="text-xs font-medium text-[#052838]">{p.name}</p>
                        <p className="text-[10px] font-sans text-[#5a8898] mt-0.5">{p.age}y · {p.phone} · {p.total_visits} visits</p>
                      </button>
                    ))}
                  </div>
                )}
          ...
          {threadId && (
            <div
              ...
              className="text-[9px] font-sans text-[#8aaab8] bg-[#e8f2f6] border border-[#c8dde6] rounded-[6px] px-2 py-1 max-w-[80px] truncate"
            >
              {threadId.slice(0, 8)}
            </div>
          )}
          ...
          {selectedPatient && (
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-[10px] bg-[#e8f2f6] border border-[#c8dde6] text-xs text-[#5a8898] hover:text-[#052838] hover:border-sky/30 transition-all"
              >
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-[10px] bg-[#e8f2f6] border border-[#c8dde6] text-xs text-[#5a8898] hover:text-[#052838] hover:border-teal/30 transition-all"
              >
                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-[10px] bg-[#e8f2f6] border border-[#c8dde6] text-xs text-[#5a8898] hover:text-[#052838] hover:border-[#c8dde6] transition-all"
          >
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      {/* ── Messages ─────────────────────────────────────────────────────────── */}
      <div className="flex-1 overflow-y-auto bg-[#f7fafb] border border-[#c8dde6] rounded-[14px] p-5 space-y-5 min-h-0">
        {messages.map((msg, i) =>
          msg.role === 'user'
      ...
      <div className="flex-shrink-0 mt-3">
        <div className="bg-white border border-[#c8dde6] rounded-[14px] flex items-end gap-3 px-4 py-3">
          <textarea
            ref={textareaRef}
            value={input}
            ...
            rows={1}
            className="flex-1 bg-transparent text-[#052838] placeholder-[#8aaab8] text-sm resize-none focus:outline-none leading-relaxed overflow-y-auto"
          ...
          <button
            ...
            className={cn(
              ...
              input.trim() && !loading
                ...
                : 'bg-[#e8f2f6] text-[#8aaab8] cursor-not-allowed'
            )}
          >
            {loading
              ? <Spinner size="sm" />
        ...
        <p className="text-[10px] font-sans text-[#8aaab8] text-center mt-1.5">
